...
<div class="productWrap">
	<c:if
		test="${product.normalmemberinfo.nid==principal.normalmemberinfo.nid}">
		<button id="btn-delete" class="btn btn-danger">삭제</button>
		<a href="/product/${product.id}/updateForm" class="btn btn-warning">수정</a>
	</c:if>
	<div class="productLeft">
		<div class="productImg" id="thumbnail">${product.thumbnail}</div>
	</div>
	...
	<div class="productRight">
		...
		<div class="payment">
			<button class="payment_btn" onclick="location.href='/cart/order'">바로구매</button>
			<button class="cart_btn" onclick="location.href='/cart/cart'">장바구니</button>
		</div>
	</div>
		...
		<div class="card" id="reply-list-box">
			<div class="card-body">
				<form>
					...
					<c:if
						test="${product.normalmemberinfo.nid==principal.normalmemberinfo.nid}">
						<select id="review_point" name="review_point" style="margin-right:10px;">
				...
				<button type="button" id="btn-review-save" class="btn btn-primary"
					style="width: 70px; margin-left: 10px;">등록</button>
			</div>
				...
				<c:forEach var="review" items="${product.reviews}">
					<li id="review-${review.id}"
						...
						<div>
							...
							test="${review.normalmemberinfo.nname==principal.normalmemberinfo.nname}">
